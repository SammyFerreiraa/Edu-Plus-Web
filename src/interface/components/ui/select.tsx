...
const SelectWithLabel = ({
   ...
   return (
      <div className={cn("flex flex-col gap-2", wrapperClassName)}>
         {label && (
            <label htmlFor={selectId} className={cn("text-sm font-semibold", labelClassName)}>
               {label}
            </label>
         )}

         <Select {...props}>
            <SelectTrigger id={selectId} size={size} error={!!error} className={cn("w-full", triggerClassName)}>
